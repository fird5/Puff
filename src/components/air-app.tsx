    ...
    <div className={cn("puff-scene min-h-dvh", `band-${band}`)}>
      ...
      >
        <header className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <button
              type="button"
              onClick={() => void load()}
              className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-bg-elevated text-fg transition-transform duration-150 ease-out active:scale-[0.96]"
          ...
          <div className="text-center sm:text-left">
            ...
            <div className="mt-5 rounded-2xl border border-border bg-bg-elevated p-4 text-left">
              ...
              <button
                ...
                className="mt-3 inline-flex h-11 items-center gap-2 rounded-xl bg-accent px-4 text-sm font-medium text-accent-fg transition-transform duration-150 ease-out active:scale-[0.96]"
              >
        ...
        <section className="flex flex-col gap-3">
          ...
          {loc.status === "denied" ? (
            <p className="text-sm text-fg-muted">Location blocked here. Pick a region instead.</p>
          ) : null}
          {loc.status === "away" ? (
            <p className="text-sm text-fg-muted">That pin is outside Singapore. Pick the region you care about.</p>
          ) : null}
          {loc.status === "ok" ? (
            <p className="inline-flex items-center gap-1.5 text-sm text-fg-muted">
              <MapPin className="size-3.5" />
              Mapped to {REGION_LABEL[region]} from your coordinates.
            </p>
          ) : null}
            ...
            {REGIONS.map((id) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "min-h-14 rounded-xl border px-3 py-2 text-left transition-transform duration-150 ease-out active:scale-[0.96]",
                    active ? "border-fg bg-fg text-bg" : "border-border bg-bg-elevated text-fg",
                  )}
                >
                  <span className="block text-sm font-medium">{REGION_LABEL[id]}</span>
                  <span className={cn("text-xs tabular-nums", active ? "text-bg/70" : "text-fg-subtle")}>
                    {value ?? "—"}
                  </span>
                </button>
              );
                ...
                <li key={item.label} className="flex items-start gap-3">
                  <span
                    className={cn(
                      ...
                      item.ok ? "bg-good/15 text-good" : "bg-unhealthy/12 text-unhealthy",
                    )}
                  >
          ...
          <div className="grid gap-3">
              ...
              return (
                <article key={slot.title} className="rounded-2xl border border-border bg-bg-elevated p-4">
                  <div className="flex items-start gap-3">
                    <span className="mt-0.5 inline-flex size-8 shrink-0 items-center justify-center rounded-lg bg-bg-subtle text-fg">
            ...
            {weather ? (
              <p className="px-1 text-xs text-fg-subtle">
                ...
                {data?.weatherValidUntil ? ` · until ${formatSgt(data.weatherValidUntil)}` : ""}
              </p>
            ) : null}
          </div>
        </section>
...
function XLogo() {
  return (
    <svg viewBox="0 0 24 24" className="size-3.5" aria-hidden="true">
      <path
        fill="currentColor"
        d="M18.244 2H21.5l-7.5 8.57L22.5 22h-6.59l-5.16-6.74L5.2 22H1.93l8.02-9.16L1.5 2h6.76l4.66 6.17L18.244 2Zm-1.16 18.02h1.8L6.99 3.87H5.06l12.02 16.15Z"
      />
    </svg>
  );
}
